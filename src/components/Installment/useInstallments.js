...
const useInstallments = () => {
  ...
  const [installments, setInstallments] = useState(() =>
    generateInstallments(initialCount, initialAmount)
  );
    ...
    setInstallments((prev) => {
      ...
      if (prevInstallment && prevInstallment.dueDate) {
        ...
        if (selectedDate <= prevDate) {
          showToastMessage(
            `Installment #${index + 1} must be after ${
      ...
      if (nextInstallment && nextInstallment.dueDate) {
        ...
        if (selectedDate >= nextDate) {
          showToastMessage(
            `Installment #${index + 1} must be before ${
      ...
      if (isDuplicate) {
        ...
        showToastMessage(
          "This date is already selected for another installment."
        );
        return prev;
      }
